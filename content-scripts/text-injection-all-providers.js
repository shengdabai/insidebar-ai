...
(function() {
  ...
  const PROVIDER_SELECTORS = {
    ...
    // Copilot uses multiple possible input selectors depending on version/layout
    copilot: [
      'textarea#userInput',
  ...
  function detectProvider() {
    ...
    if (hostname.includes('chatgpt.com') || hostname.includes('openai.com')) {
      ...
      return 'google';
    } else if (hostname.includes('copilot.microsoft.com') || (hostname.includes('bing.com') && window.location.pathname.startsWith('/chat'))) {
      return 'copilot';
    }
    return null;
  }
  ...
  function injectTextIntoElement(element, text) {
    ...
    try {
      ...
      if (isTextarea) {
        ...
        // For contenteditable elements - use execCommand to preserve formatting
        element.focus();
  ...
  function handleTextInjection(event) {
    // Origin verification: only accept messages from the same window or chrome-extension
